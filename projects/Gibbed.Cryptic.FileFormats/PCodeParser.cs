...
namespace Gibbed.Cryptic.FileFormats
{
    public static class PCodeParser
    {
        ...
        private static readonly Dictionary<MultiValueOpcode, string> _SimpleStatements =
            new Dictionary<MultiValueOpcode, string>()
            {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                else
                {
                    switch (item.Op)
                    {
                        ...
                        {
                            sb.AppendFormat("int {0}", (long)item.Arg);
                            break;
                        }

                        case MultiValueOpcode.FLT:
                        {
                            sb.AppendFormat("float {0}", (double)item.Arg);
                            break;
                        }

                        case MultiValueOpcode.STR:
                        {
                            sb.AppendFormat("str \"{0}\"", Escape((string)item.Arg));
                            break;
                        }

                        case MultiValueOpcode.OBJ:
                        {
                            sb.AppendFormat("objpath \"{0}\"", Escape((string)item.Arg));
                            break;
                        }

                        case MultiValueOpcode.RP_:
                        {
                            sb.AppendFormat("rootpath \"{0}\"", Escape((string)item.Arg));
                            break;
                        }

                        case MultiValueOpcode.IDS:
                        {
                            sb.AppendFormat("ident \"{0}\"", Escape((string)item.Arg));
                            break;
                        }

                        case MultiValueOpcode.FUN:
                        {
                            sb.AppendFormat("call \"{0}\"", Escape((string)item.Arg));
                            break;
                        }
                        ...
                        case MultiValueOpcode.S_V:
                        {
                            ...
                            sb.AppendFormat("static {0}", arg);
                            break;
                        }

                        case MultiValueOpcode.J__:
                        {
                            sb.AppendFormat("j {0}", labels[(long)item.Arg]);
                            break;
                        }

                        case MultiValueOpcode.JZ_:
                        {
                            sb.AppendFormat("jz {0}", labels[(long)item.Arg]);
                            break;
                        }
        ...
        private static readonly Dictionary<string, MultiValueOpcode> _SimpleOps =
            new Dictionary<string, MultiValueOpcode>()
            {
