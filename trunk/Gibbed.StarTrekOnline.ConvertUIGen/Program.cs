        ...
        public static void Main(string[] args)
        {
            ...
            if (mode == Mode.ToXML)
            {
                ...
                using (var input = File.OpenRead(inputPath))
                {
                    ...
                    if (blob.ParserHash != 0xFEADFED1)
                    {
                        Console.WriteLine("Invalid parser hash, new version of UIGen?");
                        return;
                    }
                    ...
                    using (var index = File.Create(Path.Combine(outputPath, "@blob.xml")))
                    {
                        ...
                        {
                            var serializer = new DataContractSerializer(typeof(UIGen.UIGen));

                            Console.WriteLine("Loading entries...");
                            var entries = BlobReader.LoadResource<UIGen.UIGen>(input);

                            var fileNameCounters = new Dictionary<string, int>();
                            var fileNames = entries.Select(e => e.FileName);
                            foreach (var fileName in fileNames.Distinct())
                            {
                                if (fileNames.Where(n => n == fileName).Count() > 1)
                                {
                                    fileNameCounters.Add(fileName, 0);
                                }
                            }
            ...
            else if (mode == Mode.ToBIN)
            {
                ...
                var entries = new List<UIGen.UIGen>();
                var serializer = new DataContractSerializer(typeof(UIGen.UIGen));
                ...
                using (var index = File.OpenRead(Path.Combine(inputPath, "@blob.xml")))
                {
                    ...
                    while (entryNodes.MoveNext() == true)
                    {
                        ...
                        using (var input = File.OpenRead(entryPath))
                        {
                            ...
                            var entry = (UIGen.UIGen)serializer.ReadObject(reader);
                            entries.Add(entry);
                        }
                    }
                }
